...
*Azure VM Compute Options*: Azure VM, AKS, Azure Container Instance, Azure Container Apps, Azure Virtual Desktop.
...
There is also **Azure Reserved VM Instance** - you commit to a VM size, region and term (1-3 years). You pay upfront or monthly but you get at much lower rate (up to 72% savings). No flexibility but it is ideal for stable, always-on workloads and long-term production systems.

**Virtual machine scale sets** let you create and manage a group of identical, load-balanced VMs. If I need multiple instances but don’t want to manually create everyhting. If I want to scale multiple identical virtual machines. Focus = scalability. **Virtual machine availability sets**  are designed to ensure that VMs stagger updates and have varied power and network connectivity, preventing you from losing all your VMs with a single network or power failure. Availability sets do this by grouping VMs in two ways: update domain and fault domain. Focus = resiliency and availability.

While virtual machines are an excellent way to reduce costs versus the investments that are necessary for physical hardware, they're still limited to a single operating system per virtual machine. If you want to run multiple instances of an application on a single host machine, containers are an excellent choice. Containers are a virtualization environment. Much like running multiple virtual machines on a single physical host, you can run multiple containers on a single physical or virtual host. Unlike virtual machines, you don't manage the operating system for a container. **Azure Container Instances** are fastest and simplest way to run container in Azure. You can get your own VM, install container runtime etc. but there is Azure Container Instances that is doing all of that for us. This is scenario to run isolated containers, without orchestration. **Azure Kubernetes Service (AKS)** is a container orchestration service. An orchestration service manages the lifecycle of containers. When you're deploying a fleet of containers, AKS can make fleet management simpler and more efficient. Management is fully managed. What we see is container hosts, nodes in our subscription, that is where containers are running. It creates actually a VM Scale Set. I can focus on deployment yaml files. You pay only for agent nodes not master node.
...
If you need to host your application on Azure, you might initially turn to a virtual machine (VM) or containers. There are other hosting options that you can use with Azure, including **Azure App Service** - an HTTP based service for hosting web apps, REST APIs, mobile backends. App Service enables you to build and host web apps, background jobs, mobile back-ends, and RESTful APIs in the programming language of your choice without managing infrastructure. It offers automatic scaling and high availability. What I pay for is the SKU. 

*Azure Networking options*: Virtual network, Virtual Subnet, VPN Gateway, VNET Peering, Express Route

Networking in Azure is related to boundaries, isolation and segmentation, Internet communication, route network traffic, filter network traffic. **Azure virtual networks** - logical representation of your network in Azure. Azure Virtual Network and virtual subnets enable Azure resources, such as VMs, web apps, and databases, to communicate with each other, with users on the internet, and with your on-premises client computers. Azure virtual networking supports both public (can be accessed from anywhere in the world) and private endpoints (exist within a virtual network and have private IP address from address space of that virtual network) to enable communication between external or internal resources with other internal resources. We can have multiple subnets inside of virtual network. Virtual networks are IaaS type of Cloud Service. It enables to create a dedicated private cloud-only network. **VNET Peering** enables connection of 2 or more Virtual Networks in Azure and usually we connect networks in Azure that way.

**Virtual Subnet** - segment address space of VNET to create sub-networks. Allows Azure resouce deployment into specific subnet, example: databases and database servers deployed to certain subnet and application to their subnet. That is called network segmentation.
...
A **virtual private network (VPN)** uses an encrypted tunnel within another network. VPNs are typically deployed to connect two or more trusted private networks to one another over an untrusted network (typically the public internet). A VPN gateway is a type of virtual network gateway. **Azure VPN Gateway** - sends encrypted traffic between an Azure VNET and on-premises location over the Internet. Instances are deployed in a dedicated subnet of the virtual network. When setting up a VPN gateway, you must specify the type of VPN - either policy-based or route-based. The primary distinction between these two types is how they determine which traffic needs encryption. Policy based is static and we want to used route-based almost always.

**Azure ExpressRoute** lets you extend your on-premises networks into the Microsoft cloud over a private connection, with the help of a connectivity provider. This connection is called an ExpressRoute Circuit. Traffic does NOT go through internet (different to site-to-site VPN). With ExpressRoute, you can establish connections to Microsoft cloud services, such as Microsoft Azure and Microsoft 365. This allows you to connect offices, datacenters, or other facilities to the Microsoft cloud. Each location would have its own ExpressRoute circuit. ExpressRoute connections don't go over the public Internet. This allows ExpressRoute connections to offer more reliability, faster speeds, consistent latencies, and higher security than typical connections over the Internet.

**Azure DNS** is a hosting service for DNS domains that provides name resolution by using Microsoft Azure infrastructure. Benefits: reliablity and performance, security, ease of use, alias records. Can provide both internal and external DNS.

**Service Endpoint** - lock down access to all instances of a PaaS service to a VNET. Accessible from internet. 
**Private Endpoint** - grants access to a specific instance (resource) of a PaaS service in your VNET on a private IP address. Enables access from on-premises without public endpoint.

![Network concepts in Azure](img/network_concepts.png) <br>

Network security concepts: Defense in-Depth, Network Security Group, Azure Firewall.

**Defense in-Depth** - layered approach, multiple methods to completely protect.
**Network Security Group (NSG)** - contain security rules that allow or deny inbound traffic to or outbound network traffic from. For each rule you specify source and destination port and protocol. Can be applied to a subnet or network adapter.
**Azure Firewall** - managed, cloud-based network security service that protects your Azure Virtual Network resources. Fully stateful firewall as a service (you don't need to manage it) with built-in high-availability and scalability.
**Azure DDoS** - provide DDoS attack defense. Standard tier provides enhanced DDoS attack defense. In free basic tier some of the features like logging, alerting and telemetry are not included but Azure DDoS is included by default.
...
The **Azure storage** platform is Microsoft's cloud storage solution for modern data storage scenarios. It offers highly available, massively scalable, durable, and secure storage for a variety of data objects in the cloud. Azure storage: durable, highly available, secure, scalable, managed, accessible. An **Azure storage account** contains all of your Azure Storage data objects: blobs, files, queues, and tables. The storage account provides a unique namespace for data and it is accessible from anywhere in the world over HTTP or HTTPS. Every object that you store in Azure Storage has a URL address that includes your unique account name. The combination of the account name and the service endpoint forms the endpoints for your storage account e.g.: *https://<storage-account-name>.blob.core.windows.net*. Names must be between 3-24 letters and may contain numbers and lowercase letters only and must be unique.

Redundancy options when creating storage account: 
- Locally redundant storage (LRS) - replicates your storage account three times within a single data center in the primary region. Lowest-cost of redundancy. Protects from server rack and drive failures.
- Zone-redundant storage (ZRS) - replicates your storage account synchronously across three Azure availability zones in the primary region. You need to use Availability-Zone enabled regions.
...
With GRS and GZRS redundancy is extended to the secondary region. GZRS is getting also physical protection of availability zones. Azure is recommending GZRS for apps requiring high-availability.

Azure Storage services: Blobs, Files, Queues, Disks, Tables.

Storage tiers: 
- Hot - most expensive to store but access cost is lowest. Data is accessed or modified frequently.
- Cool - infrequently accessed or modified. Should be stored a minimum of a 30 days.
- Cold - rarely accessed or modified but still requires fast retrieval. Should be stored a minimum of a 90 days.
- Arhive - Lowest storage but high access cost. Offline tier. Data should be stored for a minimum of 180 days. First three tiers are online tiers.

**Azure Blobs** - storage is an object storage solution for the cloud. It can store massive amounts of data, such as text or binary data. Azure Blob storage is unstructured, meaning that there are no restrictions on the kinds of data it can hold, can not be contained in row-column database. Blob storage can manage thousands of simultaneous uploads, massive amounts of video data, constantly growing log files, and can be reached from anywhere with an internet connection. Use cases: serving images or documents to a browser, streaming video and audio etc. Access tiers: Hot tier (accessed or modified frequently), Cool tier (not frequently e.g.: invoices - once a month), Cold tier (infrequently accessed but requires fast retrieval), Archive tier (rarely accessed, e.g.: long-term backups).

**Azure File** - storage offers fully managed file shares in the cloud that are accessible via the industry standard Server Message Block (SMB) or Network File System (NFS) protocols. NFS and SMB are network file sharing protocols. NFS - primarily for Linux and UNix and SMB for Windows. Benefits of Azure File: shared access, fully managed etc.
...
1. Verify explicitly - always authenticate and authorize identites based on all available data points - signals
2. Use least privilege access - limit user access
3. Assume breach - segment access to minimize blast radius

Difference to a classic approach where everything is restrited to a secure network - Zero Trust protects assets anywhere with central policy based on signals.
...
**Microsoft Defender for Cloud** is a monitoring tool for security posture management and threat protection. It monitors your cloud, on-premises, hybrid, and multi-cloud environments (other clouds also GCP, AWS..) to provide guidance and notifications aimed at strengthening your security. Previously known as Azure Security Center. Integrates with Azure Policy. Microsoft Defender for Cloud is a multicloud security solution. Because Defender for Cloud is an Azure-native service, many Azure services are monitored and protected without needing any deployment. 
